...
class OnboardingViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupViews()
        setConstraints()
        updateSkipButtonVisibility() // Инициализируем видимость кнопки
    }
    ...
    private func updateUI() {
        DispatchQueue.main.async { [self] in
            ...
            updatePageIndicators()
            updateSkipButtonVisibility() // Обновляем видимость кнопки при смене страницы
        }
    }
    
    // Функция для обновления видимости кнопки Skip
    private func updateSkipButtonVisibility() {
        // Скрываем кнопку на последней странице (currentPage == 2)
        skipButton.isHidden = (currentPage == numberOfPages - 1)
        
        // Анимация для плавного скрытия/показа
        UIView.animate(withDuration: 0.3) {
            self.skipButton.alpha = (self.currentPage == self.numberOfPages - 1) ? 0 : 1
        }
    }
